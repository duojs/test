...
Saucelabs.prototype.start = function*(name, debugmode){
  var name = name || 'test';
  var app = this.app;
  var self = this;
  var gens = [];
  ...
  this.browsers.forEach(function(b){
    gens.push(function*(){
      ...
      var browser = conf.browserName + ' ' + (conf.version || 'latest');

      debug('connect %s - %s', conf.name, browser);
      yield conn.init(conf);
      debug('connected %s - %s', conf.name, browser);
  ...
  yield parallel(gens, gens.length);
  app.destroy();
};
